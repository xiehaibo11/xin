...
(function ($) {
    ...
    var _Modal = function(options) {
        ...
        //标题
        var modal_dialog_header = $('<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 class="modal-title">' + options.title + '</h4></div>').appendTo(modal_dialog_content);

        //内容
        var modal_dialog_body = $('<div class="modal-body"></div>').appendTo(modal_dialog_content);
        modal_dialog_body.html(options.html);
        //按钮-底部
        var modal_dialog_footer = $('<div class="modal-footer"><button type="button" class="btn btn-default" data-dismiss="modal">' + options.closeBtnText + '</button></div>').appendTo(modal_dialog_content);
