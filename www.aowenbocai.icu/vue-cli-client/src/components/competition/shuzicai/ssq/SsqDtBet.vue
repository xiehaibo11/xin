    ...
    <div>
            ...
            <div class="fore">
                ...
                <div>
                    <div class="flex-box head" @click="foreShowDm = !foreShowDm">
                        <span class="c-2">红球区胆码</span>
                        <span class="f-sm flex tl ml-sm c-3">我认为必出的号码<em class="red">(至多5个)</em></span>
                    ...
                    <div class="fore-bet-wrapper bet-box" v-show="foreShowDm">
                        <ul class="balls-box">
                            <li v-for="(n,i) in foreBall">
                                <a class="balls" :class="{'selected' : checkForePlan('dm',n)}" @click="doBetDm(1,n,5)">{{n}}</a>
            ...
            <div class="after">
                ...
                <div class="mt">
                    <div class="flex-box head" @click="backShowTm = !backShowTm">
                        <span class="c-2">篮球区拖码</span>
                        <span class="f-sm flex tl ml-sm c-3">我认为可能出的号码<em class="red">(至少1个)</em></span>
        ...
        <div class="bet-foot">
            <div class="yl-box f-mini border-top-1px tc c-3">
                <template v-if="!notes">
                    红球区胆码<em class="red">{{f_dm_n}}</em>个,拖码<em class="red">{{f_tm_n}}</em>个,
                    篮球区胆码<em class="red">{{b_dm_n}}</em>个,拖码<em class="red">{{b_tm_n}}</em>个
                </template>
                <template v-else>
    ...
    export default {
        ...
        data () {
            return {
                ...
                forePlanTm:[],//红球区拖码投注数据
                backPlanDm:[],//篮球区胆码投注数据
                backPlanTm:[],//篮球区拖码投注数据
            }
        },
        computed:{
            lotteryUnit(){
            ...
            f_tm_n(){
                return this.forePlanTm.length  //红球区拖码数量
            },
            b_dm_n(){
                return this.backPlanDm.length  //篮球区胆码数量
            },
            b_tm_n(){
                return this.backPlanTm.length  //篮球区拖码数量
            },
            //计算注数
            notes(){
                if(!this.b_dm_n && !this.f_dm_n || this.f_dm_n + this.f_tm_n < 7 || this.b_tm_n < 1){
                    return 0
                }else {
                    let foreNote = this.$bet.combination(this.f_tm_n,6 - this.f_dm_n);
                    let backNote = this.$bet.combination(this.b_tm_n,1 - this.b_dm_n);
            ...
            addBetBasket(){
                if(this.notes < 1){
                    if(!this.b_dm_n && !this.f_dm_n){
                        this.$messagebox('提示','红球区和篮球区胆码不能同时为空！')
                        return
                    }
                    if(this.f_dm_n + this.f_tm_n < 7){
                        this.$messagebox('提示','红球区胆码和拖码的总个数至少7个！')
                        return
                    }
                    if(this.b_tm_n < 1){
                        this.$messagebox('提示','篮球区至少选择1个拖码！')
                        return
                    }
                    return
                }
