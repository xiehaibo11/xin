    ...
    <div class="copy-box">
        <div class="contentH" :style="{height:contentH + 'px'}">
            <div class="bet-tips f-mini c-3 bd-v">从01-11中手动输入{{n}}个号码进行购买，从01-11共11个号码中选择{{n}}个号码进行购买,
                <template v-if="type < 8 || type == 13">与开奖号码任意<em v-if="n > 5">5</em><em v-else>{{n}}</em>个数字相同，</template>
                <template v-if="type ==8">与开奖号码第1个数字相同，</template>
                <template v-if="type ==9">与开奖号码的前两位号码相同（顺序不限），</template>
                <template v-if="type ==10">与开奖号码的前两位号码相同且顺序一致，</template>
                <template v-if="type ==11">与开奖号码的前三位号码相同（顺序不限），</template>
                <template v-if="type ==12">与开奖号码的前三位号码相同且顺序一致，</template>
...
<script>
    ...
    export default {
        ...
        computed:{
            ...
            //选号容器高度计算
            contentH(){
                let h = this.rebateIsOpen && this.isRebate || this.unitIsOpen ?  89 : 52 //底部高度
//                let a = this.notes > 0 && this.betModel == 1 ? 30 : 0
        ...
        methods:{
            ...
            addNum(betNum){
                if(this.type == 10 || this.type == 12){ //直选
