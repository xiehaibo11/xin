<template>
    <div>
        ...
        <div class="dlt-wrapper">
            ...
            <!--标准选号-->
            <plw-zhix-fs v-if="type==1" :type="type" text="标准选号" :miss="miss" :expect="initData.expect" :gain="gain" :title="title" :end-time="initData.end_time"></plw-zhix-fs>
            <!--单式投注-->
            <plw-copy-bet v-if="type==2" :type="type" text="单式投注" :expect="initData.expect" :gain="gain" :title="title" :end-time="initData.end_time" :n="5"></plw-copy-bet>
        </div>
    </div>
</template>

<script>
    import PlwZhixFs from 'components/competition/shuzicai/PlZhixFs.vue' //排列3-直选复式
    import PlwCopyBet from 'components/competition/shuzicai/pls/PlsCopyBet.vue' //排列3-单式
    export default {
        name: 'plw',
        components:{
            PlwZhixFs,
        ...
        created(){
            ...
            this.timeInterval();
            this.$store.commit('setKeepAlivePage','plw')
        },
        //离开当前投注页面
        beforeRouteLeave(to, from, next){
            if(to.path =='/shuzicai/bet'){
                ...
                this.$store.commit('delKeepAlivePage','plw') //清除投注页面缓存
