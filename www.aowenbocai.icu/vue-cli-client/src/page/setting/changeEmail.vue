    ...
    <div>
        ...
        <div class="step1" v-if="steps == 1">
            ...
            <div v-show="checkWay == 1">
                ...
                <safe-check url="/web/user/sendMsgEmail" v-model="telYzm" :check-way="1"></safe-check>
            </div>
            <!--验证邮箱-->
            <div v-show="checkWay == 2">
                <div class="input-title">安全验证</div>
                <safe-check url="/web/user/sendOldEmail" v-model="emailYzm" :check-way="2"></safe-check>
            </div>
            <div class="btn-box">
        ...
        <div class="step2" v-if="steps == 2">
            <div class="input-title">重置邮箱</div>
            <get-sms :get-way="2" url="/web/user/sendNewEmail" v-model="options"></get-sms><!--重置邮箱-->
            <div class="btn-box">
...
<script>
    ...
    export default {
        ...
        data () {
            return {
                checkWay:Number(this.$store.state.setting.tel_checked) && this.$store.state.lockInfo.tel !== 1 ? 1 : 2 , //验证方式 优先短信验证
                loading:false,
        ...
        methods:{
            toNext(){ //下一步
                ...
                this.loading = true
                this.$axios.post('/web/user/checkOldEmail',{
            ...
            sumbit(){
                ...
                this.$axios.post('/web/user/checkNewEmail',data).then(({data}) =>{
                    if(!data.err){
                        setTimeout(()=>{
