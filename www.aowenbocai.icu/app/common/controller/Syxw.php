...
class Syxw extends UserBase
{
    public function index($name)
    {
        ...
        if (!$system['rebate_isOpen']) {//返点关闭  奖金扣除系统盈利
            ...
            foreach ($play as &$v2) {
                $v2 = $model->setGain($v2, ($bonus_base + $user_base));
            }
            $play = collection($play);
            if (isset($newArr['gain'])) {
                $newArr = $model->setGain($newArr, ($bonus_base + $user_base));
   ...
   /***这里面所有有关彩种的引用均是可变的，后期统一后再做调整*/
    ...
    public function getLotteryCommon($name)
    {
        ...
        foreach ($bouns as $key => $value) {
            ...
            if(!isset($value['type']) || !$value['type']){
                ...
                $small[$sign[0]][$sign[1]] = $value['isOpen'] ? 1 : 0;
            }
        }
        ...
        if(strval($issue['lastIssue']) != strval($code['expect'])){
            $code = ['code' => '0,0,0,0,0', 'expect' => $issue['lastIssue']];
        ...
        if(session('?'.$name.'_getnewcode')){
            array_unshift($tenCode,['expect' => $issue['lastIssue'], 'code' =>['开','奖','中','.','.'], 'dx' => '--', 'jo'=>'--','hz' => '--']);
        ...
        return ['title' => $title['title'], 'lottery' => $title, 'play' => collection($play),'small' => collection($small), 'info' => $newArr, 'cold' => collection($hot['cold']),
            'hot' => collection($hot['hot']), 'ten' =>collection($tenCode), 'open' => collection($open),'getnewcode' => session('?'.$name.'_getnewcode'),'miss' => $miss,'openTime' => $openTime,'name'=>$name,
            'firstIssue' => $fristExp,
    ...
    /**ajax获取最新开奖号码 */
    public function getModel($name = 'gd11')
    {
        return LotteryCommon::getModel($name, 'code');
    }

    /**获取期号 */
    public function getIssueInfo($name = 'gd11')
    {
        /**判断该彩种是否存在并开启 */
    ...
    /**ajax获取遗漏 */
    public function getMiss($name = 'gd11')
    {
    ...
    /**获取冷热号 */
    public function getHot($name = 'gd11', $num = 100)
    {
        $codeClass = $this->getModel($name);
        ...
        foreach ($code as $key => $value) {
            ...
            foreach ($valCode as $val){ 
                $val = intval($val) < 10 ? '0'.intval($val) : $val;
                $codeNum[$val] +=1;
            }
        }
        asort($codeNum);
    ...
    /**ajax获取最新开奖号码 */
    public function getNewCode($issue, $name = 'gd11')
    {
        $codeClass = $this->getModel($name);
    ...
    public function setCode2($code)
    {
        ...
        foreach ($code['code'] as $value) {
            $_value = intval($value);
            $_value > 5 ? $da++ : $xiao++;
            $_value % 2 == 0 ? $shuang++ : $dan++;
            $sum += $_value;
        }
        $code['dx'] = $da.":".$xiao;
        $code['jo'] = $dan.":".$shuang;
