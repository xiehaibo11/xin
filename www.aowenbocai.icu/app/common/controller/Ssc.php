  ...
  private  function get_url($url)
{
     ...
     */
    private function notifyApiError($name, $error)
    {
        // 可以发送邮件、短信或记录到特殊日志文件
        $message = "彩种 {$name} API调用异常: {$error}，时间: " . date('Y-m-d H:i:s');
        \think\Log::error("API异常通知: " . $message);

        // 示例：写入特殊的错误文件
        $errorFile = ROOT_PATH . 'runtime/api_errors.log';
        ...
        // var_dump($name);
         if($name=='ynssc')
         {
             try {
                 ...
                 // 记录成功日志
                 \think\Log::info("ynssc API调用成功，期号: {$preissue}");

             } catch (\Exception $e) {
                 // 记录错误日志
                 \think\Log::error("ynssc API调用失败: " . $e->getMessage() . ", URL: {$url}");
                 ...
                 // 可选：通知管理员API异常
                 // $this->notifyApiError($name, $e->getMessage());
             }
         }
               ...
               if($name=='jlssc')
         {
             try {
                 ...
                 $preissue = '20' . $result[0]['preDrawIssue'];
                 if (!is_numeric($result[0]['preDrawIssue'])) {
                     throw new \Exception('preDrawIssue不是有效的数字');
                 }
                 ...
                 // 记录成功日志
                 \think\Log::info("jlssc API调用成功，期号: {$preissue}");

             } catch (\Exception $e) {
                 // 记录错误日志
                 \think\Log::error("jlssc API调用失败: " . $e->getMessage() . ", URL: {$url}");
	...
	public function getLotteryInfo($name)
	{
        ...
        if (!$system['rebate_isOpen']) {//返点关闭  奖金扣除系统盈利
         ...
         if($name=='ynssc')
         {
          ...
          $newArr['lastIssue']= intval($preissue); 
          $newArr['expect']=intval($preissue)+1;  
          $newArr['sort_expect']= intval($preissue)+1;  
    ...
    public function getLotteryCommon($name)
    {
        ...
        $tenCode = $this->setCode($tenCode, 10,$name);
    ...
    /**获取期号 */
    public function getIssueInfo($name = 'ssc')
    {
        /**判断该彩种是否存在并开启 */
       ...
       // if($name!='ynssc')
        $openTime = (new Lottery())->issueToTime($name, $issue['expect']);
        //else  $openTime=strtotime(date('Y-m-d 21:10:00'));
        //file_put_contents('openTime',$openTime);
        ...
        $result=$data['result']['data'];
        $preissue=$result[0]['preDrawIssue'];
        $issue['lastIssue']= intval($preissue); 
        $issue['expect']=intval($preissue)+1;  
        $issue['sort_expect']= intval($preissue)+1;  
    ...
    /**ajax获取遗漏 */
    public function getMiss($name = 'ssc')
    {
    ...
    /**获取冷热号 */
    public function getHot($name = 'ssc', $num = 100)
    {
        $codeClass = $this->getModel($name);
    ...
    /**ajax获取最新开奖号码 */
    public function getModel($name = 'ssc')
    {
        return LotteryCommon::getModel($name, 'code');
    }

    /**ajax获取最新开奖号码 */
    public function getNewCode($issue, $name = 'ssc')
    {
        ...
        if(!$res){
           ...
           $data['lastIssue']=$tenCode[0]['expect'];
            return json(['err' => 0, 'data' => $data]);
        }
    }

    /**开奖号码设置形态 */
    public function setCode($code, $num,$name)
    {
        if(!$code){
            return [];
        }
        ...
        $code = $code->toArray();
        if($num == 1){
            $code = $this->setCode2($code,$name);
        }else{
            foreach ($code as &$value) {
                $value =$this->setCode2($value,$name);
            }
        }
        return $code;
    }
    /**开奖号码设置形态 2*/
    public function setCode2($code,$name)
    {
        $sum = 0;
        $da = 0;
        $xiao = 0;
        ...
        if(count($code['code']) != 5&&$name!='ynssc'&&$name!='jlssc'){
			$code['hs'] = '--';
			$code['shi'] = '--';
			$code['ge'] = '--';
			return $code;
