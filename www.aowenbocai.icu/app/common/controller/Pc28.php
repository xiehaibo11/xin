...
use app\lottery\model\Pc28 as ASetting;
   ...
   /***这里面所有有关彩种的引用均是可变的，后期统一后再做调整*/
    ...
    public function getLotteryCommon($name)
    {
        ...
        if(strval($issue['lastIssue']) != strval($code['expect'])){
            $code = ['code' => '-1,-1,-1', 'expect' => $issue['lastIssue']];
        ...
        if(session('?'.$name.'_getnewcode')){
            array_unshift($tenCode,['expect' => $issue['lastIssue'], 'code' =>['开','奖','中'], 'xt' => '--', 'he'=>'?']);
    ...
    /**获取期号 */
    public function getIssueInfo($name = 'bj28')
    {
        /**判断该彩种是否存在并开启 */
    ...
    /**ajax获取最新开奖号码 */
    public function getModel($name = 'bj28')
    {
        return LotteryCommon::getModel($name, 'code');
    }

    /**ajax获取遗漏 */
    public function getMiss($name = 'bj28')
    {
    ...
    /**获取冷热号 */
    public function getHot($name = 'bj28', $num = 100)
    {
        $codeClass = $this->getModel($name);
    ...
    /**ajax获取最新开奖号码 */
    public function getNewCode($issue, $name = 'bj28')
    {
        $codeClass = $this->getModel($name);
