...
class Page {
    ...
    protected $coolPages   ;
    // 分页显示定制
    protected $config  =    array('header'=>'条记录','prev'=>'上一页','next'=>'下一页','first'=>'第一页','last'=>'最后一页','theme'=>'<ul class="pagination"><li class = "page_total"><span>%totalRow% %header% <input class ="pageInputNum" value ="%nowPage%"><button class="btn btn-xs btn-info pageTurn">跳转</button>/%totalPage% 页</span></li> %upPage% %downPage% %first%  %prePage%  %linkPage%  %nextPage% %end% </ul>');
    // 默认分页变量名
    protected $varPage;
    ...
    {
        return ['pages' => $this->totalPages,'rows' => $this->totalRows,'now' => $this->nowPage, 'pagesize' => $this->listRows];
    }
    /**
     * 分页显示输出
     * @access public
    ...
    public function show() {
        ...
        if($this->url){
            ...
            $url =  $path."?".urldecode(http_build_query($parameter, null, '&'));
        }
        //上下翻页字符串
        ...
        if(isset($turnParameter['page'])){
            $key = array_keys($turnParameter);
            $index = array_search('page', $key);
            $turnParameter = array_splice($turnParameter,0, $index);
        }
        $turnUrl =  $path."?".urldecode(http_build_query($turnParameter, null, '&'));
        $pageStr     =   str_replace(
            ...
            $pageStr .= '<script>
                $(".pageTurn").click(function(){
                    var pageNum = $(".pageInputNum").val();
                    location.href = "'.$turnUrl.'&page="+pageNum
                })
           </script>';
        return $pageStr;
    }
